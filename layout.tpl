...
<html lang="{$core.language.iso}" dir="{$core.language.direction}">
	<head>
		...
		{ia_print_js files='_IA_TPL_classie, _IA_TPL_app' order=999}
	...
	<body class="page-{$core.page.name}{if 'index' != $core.page.name} navbar--padtop{/if}">
		<header class="header"{if $core.config.website_bg} style="background-image: url('{$core.page.nonProtocolUrl}uploads/{$core.config.website_bg}');"{/if}>
			<nav class="navbar navbar-default navbar-fixed-top{if 'index' != $core.page.name} navbar-default-inner{/if}">
				<div class="container">
		...
		{if isset($iaBlocks.header1) || isset($iaBlocks.header2) || isset($iaBlocks.header3) || isset($iaBlocks.header4)}
			<div class="header-blocks">
				...
				<div class="container">
					<div class="row">
						<div class="{width section='header-blocks' position='header1' tag='col-md-'}">{ia_blocks block='header1'}</div>
						<div class="{width section='header-blocks' position='header2' tag='col-md-'}">{ia_blocks block='header2'}</div>
						<div class="{width section='header-blocks' position='header3' tag='col-md-'}">{ia_blocks block='header3'}</div>
						<div class="{width section='header-blocks' position='header4' tag='col-md-'}">{ia_blocks block='header4'}</div>
					</div>
				</div>
			</div>
		{/if}
